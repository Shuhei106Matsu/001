                ...
                for i, start_col in enumerate(date_indices):
                    try:
                        ...
                        while current_row < len(df):
                            ...
                            if '年' in str(df.iloc[current_row].values) or 'チューター' in row_head or 'チューター' in label: break
                            ...
                            if '生徒' in label:
                                ...
                                for ts in time_slots:
                                    ...
                                    if s1: final_data.append({"日付": date_str, "曜日": dow, "時間": ts['time'], "講師": t1, "生徒名": s1})
                                    if s2: final_data.append({"日付": date_str, "曜日": dow, "時間": ts['time'], "講師": t2, "生徒名": s2})
                                current_row += 2
                            else: 
                                current_row += 1
                    except: continue
...
if uploaded_file is not None:
    with st.spinner("データを読み込んでカレンダーを作成中..."):
        try:
            ...
            if result_df.empty:
                st.error("データが抽出できませんでした。ファイルの形式を確認してください。")
            else:
                ...
                teachers = sorted([t for t in result_df['講師'].unique() if pd.notna(t)])
                ...
                for teacher in teachers:
                    ...
                    for week in weeks:
                        ...
                        for t in all_times:
                            ...
                            for d in week:
                                s_list = t_df[(t_df['date_obj'] == d) & (t_df['時間'] == t)]['生徒名'].tolist()
                                cell_val = " / ".join(s_list) if s_list else ""
                                html_content += f'<td class="student-cell">{cell_val}</td>'
                            html_content += '</tr>'
                ...
                st.info("💡 ダウンロードしたファイルを開き、「印刷 ＞ 詳細設定 ＞ 余白なし・背景のグラフィックON」でPDFにしてください。")
